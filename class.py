...
model_path_nlp = "./distilbert-telecom-finetuned"

tokenizer = DistilBertTokenizer.from_pretrained(model_path_nlp)
model = DistilBertForSequenceClassification.from_pretrained(model_path_nlp)
...
class UserRequest(BaseModel):
    unique_id: int
    answer: str = Query(
        ..., min_length=1, max_length=4096, description="Text to classify"
    )
...
async def classify_text(user_request: UserRequest):
    ...
    try:
        ...
        inputs = tokenizer(
            answer, return_tensors="pt", padding=True, truncation=True, max_length=512
        )
...
@app.post(
    "/at-risk",
    response_model=ClassificationResponse,
    summary="Identify At-Risk Customers",
    description="Classify if a sentiment is based as an at-risk customer based on text using a fine-tuned DistilBERT model.",
)
async def classify_text_at_risk(user_request: UserRequest):
    """
    Classify the sentiment of input text as 0 or 1 to identify at-risk customers.
    ...
    try:
        ...
        # Tokenize the input text
        inputs = tokenizer_at_risk(
            answer, return_tensors="pt", padding=True, truncation=True, max_length=512
        )

        # Perform inference
        with torch.no_grad():
            outputs = model_at_risk(**inputs)

        # Get predicted class (label)
        logits = outputs.logits
        predicted_class = torch.argmax(logits, dim=1).item()

        logger.info(f"Processing At-risk {unique_id} = sentiment {predicted_class}")
